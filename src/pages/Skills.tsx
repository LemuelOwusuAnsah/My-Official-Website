...
import {
  Code2, Blocks, Database, Workflow, ArrowRight,
  Layers, Server, Cloud, Wrench, ShoppingCart, Palette, Network,
  Megaphone, Brain, Users, Award, GraduationCap, Sparkles,
} from 'lucide-react'
...
const fullstackGroups = [
  { titleKey: 'fs_group_languages', Icon: Code2, itemsKey: 'fs_lang_', count: 4 },
  { titleKey: 'fs_group_lamp', Icon: Server, itemsKey: 'fs_lamp_', count: 4 },
  { titleKey: 'fs_group_frontend', Icon: Layers, itemsKey: 'fs_front_', count: 5 },
  { titleKey: 'fs_group_backend', Icon: Database, itemsKey: 'fs_back_', count: 6 },
  { titleKey: 'fs_group_deploy', Icon: Cloud, itemsKey: 'fs_deploy_', count: 5 },
  { titleKey: 'fs_group_tools', Icon: Wrench, itemsKey: 'fs_tools_', count: 5 },
  { titleKey: 'fs_group_ecom', Icon: ShoppingCart, itemsKey: 'fs_ecom_', count: 5 },
  { titleKey: 'fs_group_design', Icon: Palette, itemsKey: 'fs_design_', count: 5 },
  { titleKey: 'fs_group_network', Icon: Network, itemsKey: 'fs_net_', count: 2 },
  { titleKey: 'fs_group_marketing', Icon: Megaphone, itemsKey: 'fs_mkt_', count: 7 },
  { titleKey: 'fs_group_ai', Icon: Brain, itemsKey: 'fs_ai_', count: 6 },
  { titleKey: 'fs_group_professional', Icon: Users, itemsKey: 'fs_pro_', count: 6 },
]

const certGroups = [
  {
    titleKey: 'cert_group_ai',
    Icon: Brain,
    bg: 'bg-[#c4b5fd] dark:bg-[#6d28d9]',
    text: 'text-[#2e1065] dark:text-[#ede9fe]',
    certs: [
      ...
      {[1, 2, 3, 4, 5].map((n) => (
        <span
          ...
          className={`inline-block w-1.5 h-1.5 rounded-full ${
            n <= level ? 'bg-current' : 'bg-current opacity-20'
          }`}
        />
      ))}
    </span>
...
export default function Skills() {
  ...
  usePageMeta({
    title: 'Skills',
    description: 'React, TypeScript, Web3, Wagmi, Viem, GraphQL, REST APIs, Agile — and a full-stack practice built on the LAMP stack.',
  })
    ...
    <div className="container-content py-16 md:py-24">
      ...
      <section ref={stackRef} className="reveal mb-28">
        ...
        <div className="flex items-center gap-3 mb-10">
          ...
          <p className="eyebrow">{t('core_stack_title')}</p>
        </div>
        ...
        <div className="grid gap-5 md:grid-cols-2">
          {stackGroups.map((group) => {
            ...
            return (
              <div
                key={group.title}
                className={`relative overflow-hidden rounded-surface ${group.bg} ${group.text} p-8 md:p-10 transition-all duration-500 hover:-translate-y-1 hover:shadow-xl`}
              >
                <div className="flex items-center gap-3 mb-8">
                  <div className="inline-flex items-center justify-center w-10 h-10 rounded-button bg-white/30 dark:bg-black/20">
      ...
      <section ref={fsRef} className="reveal mb-28">
        ...
        <div className="flex items-center gap-3 mb-4">
          <Layers className="text-lemon dark:text-lemon-dark" size={20} />
          <p className="eyebrow">{t('fullstack_title')}</p>
        </div>
      ...
      <section ref={certRef} className="reveal mb-28">
        <div className="rule mb-8" />
        <div className="flex items-center gap-3 mb-4">
          <Award className="text-lemon dark:text-lemon-dark" size={20} />
          <p className="eyebrow">{t('certifications_title')}</p>
        </div>
        <p className="font-sans text-ink-muted dark:text-ink-muted-dark mb-10 max-w-2xl">
          {t('certifications_intro')}
        </p>

        <div className="rounded-surface bg-[#c4b5fd] dark:bg-[#6d28d9] text-[#2e1065] dark:text-[#ede9fe] p-8 md:p-10 mb-10">
          <div className="flex items-start gap-4">
            <div className="inline-flex items-center justify-center w-12 h-12 rounded-button bg-white/30 dark:bg-black/20 flex-shrink-0">
              <Brain size={22} strokeWidth={1.75} />
            </div>
            <div>
              <h3 className="font-display text-2xl md:text-3xl font-semibold tracking-tight mb-3">
                {t('ai_highlight_title')}
              </h3>
              <p className="font-sans text-base md:text-lg leading-relaxed text-[#4c1d95] dark:text-[#ddd6fe]">
                {t('ai_highlight_intro')}
              </p>
            </div>
          </div>
        </div>
            ...
            return (
              <div key={group.titleKey}>
                <div className="flex items-center gap-3 mb-5">
                  <div className={`inline-flex items-center justify-center w-8 h-8 rounded-button ${group.bg} ${group.text}`}>
      ...
      <section ref={discRef} className="reveal">
        <div className="rule mb-8" />
        <div className="flex items-center gap-3 mb-10">
          <Users className="text-lemon dark:text-lemon-dark" size={20} />
          <p className="eyebrow">{t('skills_categories_title')}</p>
        </div>

        <div className="grid gap-5 md:grid-cols-3">
          <SkillCategory
            titleKey="skill_cat_tech"
            defaultTitle="Technology Skills"
            itemsKeyPrefix="skill_tech_"
            count={6}
            tone="lemon"
          />
          <SkillCategory
            titleKey="skill_cat_health"
            defaultTitle="Health & Life Sciences"
            itemsKeyPrefix="skill_health_"
            count={5}
            tone="cyan"
          />
          <SkillCategory
            titleKey="skill_cat_creative"
            defaultTitle="Creative & Communication Arts"
            itemsKeyPrefix="skill_creative_"
            count={8}
            tone="violet"
          />
        </div>

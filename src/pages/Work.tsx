...
export default function Work() {
  ...
  usePageMeta({ title: 'Work', description: 'Published books, shipped software, and original music — from Accra, Ghana.' })
  ...
  return (
    <div className="container-content py-16 md:py-24">
      <div ref={heroRef} className="reveal max-w-4xl mb-24">
        ...
        <p className="font-sans text-lg md:text-xl text-ink-muted dark:text-ink-muted-dark leading-relaxed max-w-2xl">{t('work_intro')}</p>
      </div>

      <section ref={booksRef} className="reveal mb-28">
        <div className="rule mb-8" />
        <p className="eyebrow mb-4">{t('work_books_title')}</p>
        <p className="font-sans text-ink-muted dark:text-ink-muted-dark mb-10 max-w-2xl">{t('work_books_intro')}</p>
        <a href={APPLE_BOOKS_URL} target="_blank" rel="noopener noreferrer" className="group inline-flex items-center gap-3 mb-12 px-6 py-4 rounded-surface bg-gradient-to-br from-[#fbbf24] to-[#fb923c] text-[#422006] font-medium text-sm shadow-lg transition-all duration-300 hover:-translate-y-0.5 hover:shadow-xl">
          ...
          {books.map((book) => (
            <a key={book.titleKey} href={APPLE_BOOKS_URL} target="_blank" rel="noopener noreferrer" className={`group relative overflow-hidden rounded-surface ${book.bg} ${book.text} p-8 md:p-10 min-h-[240px] flex flex-col transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl will-change-transform`}>
              <div className="relative z-10 flex flex-col h-full">
                ...
                <span className={`mt-auto pt-6 inline-flex items-center gap-2 font-mono text-2xs uppercase tracking-[0.14em] ${book.muted}`}>{t('work_read_on_apple')}<ArrowUpRight size={11} /></span>
              </div>
            </a>
          ))}
        </div>
      ...
      <section ref={projectsRef} className="reveal mb-28">
        <div className="rule mb-8" />
        <p className="eyebrow mb-4">{t('work_projects_title')}</p>
        <p className="font-sans text-ink-muted dark:text-ink-muted-dark mb-10 max-w-2xl">{t('work_projects_intro')}</p>
        <a href={GITHUB_URL} target="_blank" rel="noopener noreferrer" className="group inline-flex items-center gap-3 mb-12 px-6 py-4 rounded-surface border-2 border-ink dark:border-ink-dark text-ink dark:text-ink-dark font-medium text-sm transition-all duration-300 hover:-translate-y-0.5 hover:bg-ink hover:text-canvas dark:hover:bg-ink-dark dark:hover:text-canvas-dark">
        ...
        <div className="rounded-surface bg-[#c4b5fd] dark:bg-[#6d28d9] text-[#2e1065] dark:text-[#ede9fe] p-8 md:p-12">
          <div className="grid gap-8 md:grid-cols-12 items-end">
            <div className="md:col-span-8">
              <p className="font-mono text-2xs uppercase tracking-[0.14em] text-[#4c1d95] dark:text-[#ddd6fe] mb-6">{t('work_music_title')}</p>
              <p className="font-script text-5xl md:text-6xl mb-6 text-[#2e1065] dark:text-[#ede9fe] leading-none">Lemy Newman</p>
              <p className="font-sans text-lg md:text-xl leading-relaxed max-w-2xl text-[#4c1d95] dark:text-[#ddd6fe]">{t('work_music_intro')}</p>
            </div>
            <div className="md:col-span-4 md:justify-self-end">
              <a href={APPLE_MUSIC_URL} target="_blank" rel="noopener noreferrer" className="group inline-flex items-center gap-2 rounded-button px-6 py-3.5 font-medium text-sm bg-[#2e1065] text-[#ede9fe] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg">

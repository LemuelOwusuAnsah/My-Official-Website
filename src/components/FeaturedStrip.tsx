...
const featured = [
  {
    key: 'proj_calculoria_name',
    desc: 'proj_calculoria_desc',
  ...
  {
    key: 'proj_capital_name',
    desc: 'proj_capital_desc',
  ...
  {
    key: 'proj_athletica_name',
    desc: 'proj_athletica_desc',
  ...
  {
    key: 'proj_convertoria_name',
    desc: 'proj_convertoria_desc',
      ...
      <div className="flex items-end justify-between mb-10 gap-6 flex-wrap">
        <div>
          ...
          <h2 className="section-title max-w-xl">
            {t('featured_title', 'Things I have shipped.')}
          </h2>
        </div>
        <Link
          to="/work"
          className="link-arrow text-ink-muted dark:text-ink-muted-dark hover:text-ink dark:hover:text-ink-dark"
        >
      ...
      <div className="grid gap-5 md:grid-cols-2">
        {featured.map((item) => (
          <a
            ...
            className={`group relative overflow-hidden rounded-surface ${item.bg} ${item.text}
          ...
          >
            ...
            <div className="relative z-10 flex flex-col h-full">
              ...
              <p className={`font-sans text-sm md:text-base leading-relaxed max-w-md ${item.muted}`}>
                {t(item.desc)}
              </p>
              ...
              <span className={`mt-auto pt-6 inline-flex items-center gap-2 font-mono text-2xs uppercase tracking-[0.14em] transition-all duration-300 group-hover:gap-3 ${item.muted}`}>
                {t('featured_view_repo', 'View repository')}
